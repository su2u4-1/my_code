...
storage_list_0: list[str | int] = ["物品", "數量"]
storage_list: list[list[str | int]] = [storage_list_0]
...
class Player:
    def __init__(self, name: str, realm: int, cultivation: int, attack: int, defense: int, dodge: int, max_health: int, cultivation_speed: int, comprehension: int, money: int, spirit_stones: int):
        self.name = name
        self.realm = realm
...
class Fighter:
    def __init__(self, name: str, realm: int, cultivation: int, attack: int, defense: int, dodge: int, max_health: int):
        self.name = name
        self.realm = realm
        self.cultivation = cultivation
        self.attack = attack
...
player = Player("預設名稱", 0, 0, 1, 1, 1, 1, a, b, 1000, 100)
...
def battle(A: Fighter, B: Fighter) -> str:
    print("\n%s開始挑戰%s" % (B.name, A.name))
    print("\n%s[境界:%d，修為:%d，真元:%d，鍛體:%d，身法:%d，血氣:%d]" % (A.name, A.realm, A.cultivation, A.attack, A.defense, A.dodge, A.max_health))
    a, b = "", ""
    if A.dodge > B.dodge:
        if random.randint(1, 100) > (B.dodge / A.dodge) * 100:
            a = A.name
            b = B.name
        else:
            a = B.name
            b = A.name
    if B.dodge >= A.dodge:
        if random.randint(1, 100) > (A.dodge / B.dodge) * 100:
            a = B.name
            b = A.name
        else:
            a = A.name
            b = B.name
    while A.max_health > 0 and B.max_health > 0:
        if a == A.name:
            if A.max_health <= 0:
                break
            if B.max_health <= 0:
            ...
            if A.attack >= B.defense:
                c = int((A.attack / B.defense) * A.attack)
                round(c)
            else:
                c = int((B.attack / A.defense) * B.attack)
                round(c)
            if (A.dodge / (A.dodge + B.dodge)) * 100 > random.randint(1, 100):
                if B.max_health >= c:
                    B.max_health = B.max_health - c
                else:
                    B.max_health = 0
                time.sleep(0.75)
                print("\n%s攻擊%s造成傷害%d，%s剩餘血氣%d" % (a, b, c, b, B.max_health))
                a = B.name
                b = A.name
            else:
                time.sleep(0.75)
        ...
        if a == B.name:
            ...
            if A.attack >= B.defense:
                ...
                c = int((A.attack / B.defense) * A.attack)
                round(c)
            if (B.dodge / (A.dodge + B.dodge)) * 100 > random.randint(1, 100):
                if A.max_health >= c:
                    A.max_health = A.max_health - c
                else:
                    A.max_health = 0
                time.sleep(0.75)
                print("\n%s攻擊%s造成傷害%d，%s剩餘血氣%d" % (a, b, c, b, A.max_health))
                a = A.name
                b = B.name
            else:
                time.sleep(0.75)
    ...
    if A.max_health > 0:
        time.sleep(0.75)
        print("\n%s打贏%s了" % (A.name, B.name))
        w = A.name
    if B.max_health > 0:
        time.sleep(0.75)
        print("\n%s打贏%s了" % (B.name, A.name))
        w = B.name
    return w
...
while repeat == 0:
    ...
    player.name = input("\n請輸入玩家名稱:")
    judge(player.name)
    if player.name != "off" and player.name != "set":
        repeat = 1
        time.sleep(0.75)
        print("\n你的名字是%s" % (player.name))
        time.sleep(0.75)
...
while repeat == 1:
    ...
    while option_1 == "1":
        ...
        if option_2 == "1":
            ...
            while b > 0:
                time.sleep(1)
                print("修為增加%d" % (player.cultivation_speed))
                player.cultivation += player.cultivation_speed
                player.spirit_stones -= 1
                b = b - 1
            time.sleep(0.75)
            print("\n%s離開了%s" % (player.name, place))
        if option_2 == "2":
            time.sleep(0.75)
            print("\n%s離開了%s" % (player.name, place))
            option_1 = 0
    ...
    while option_1 == "2":
        ...
        if option_2 == "1":
            print("\n%s:師父，我今天是來請教修練上的問題" % (player.name))
            time.sleep(0.75)
            print("\n師父:徒兒阿，為師這裡有一篇為師自己的修練心得，你且拿去看一看，有不懂的再問我")
            time.sleep(0.75)
        if option_2 == "2":
            print("\n%s:師父，我今天是來請教與宗門有關的問題" % (player.name))
            time.sleep(0.75)
            print("\n師父:徒兒阿，為師這裡有一幅宗門地圖，你且拿去看一看，有不懂的再問我")
            time.sleep(0.75)
        if option_2 == "3":
            print("\n%s:師父，我今天是想跟師父切磋" % (player.name))
            if player.realm < 12:
                time.sleep(0.75)
            ...
            if player.realm >= 12:
                ...
                d = int((784 - a - b - c))
                fa = Fighter("師父", 12, 6144, a, b, c, d)  # 名稱，境界，修為，攻擊，防禦，閃避，血量
                fb = Fighter(player.name, player.realm, player.cultivation, player.attack, player.defense, player.dodge, player.max_health)
                w = battle(fa, fb)
                if w == fa.name:
                    time.sleep(0.75)
                    print("\n師父:%s你啊，成天貪玩，該靜下心好好練練了阿" % (player.name))
                    time.sleep(0.75)
                    print("\n%s是，師父(委屈)" % (player.name))
                    time.sleep(0.75)
                if w == fb.name:
                    time.sleep(0.75)
                    print("\n%s:師父，承讓了" % (player.name))
                    time.sleep(0.75)
                    print("\n師父:長江後浪推前浪阿")
                    time.sleep(0.75)
        while option_2 == "4":
            quest_list_0 = ["編號", "任務目標", "任務獎勵"]
            quest_list_1: list[str | int] = [
                1,
                "幫師父打掃洞府",
                "師父的禮物、靈石",
                "\n任務詳細資料:\n一向尊師重道的你，看到師父的洞府如此的雜亂後，決定幫師父好好整理洞府。",
            ]
            quest_list_2: list[str | int] = [
                2,
                "幫師父餵養靈寵",
                "靈獸進階丹、靈石",
                "\n任務詳細資料:\n一向愛護小動物的你，看到師父的靈寵因為師父太忙，所以餓了很久，你決定幫師父照顧一下靈寵",
            ]
            quest_list_3: list[str | int] = [
                3,
                "幫師父照顧靈植",
                "靈液、靈石",
                "\n任務詳細資料:\n一向喜愛植物的你，看到師父的靈植因為師父太忙，所以都快乾枯了，你決定幫師父照料一下靈植",
            ]
            quest_list_4: list[str | int] = [
                4,
                "幫師父提煉礦石",
                "礦石精華、靈石",
                "\n任務詳細資料:\n一向尊師重道的你，看到師父如此忙碌，所以你決定幫師父提煉一下礦物",
            ]
            quest_list_5: list[str | int] = [
                5,
                "幫師父收集妖獸內丹",
                "妖獸精華、修為丹、靈石",
                "\n任務詳細資料:\n",
            ...
            time.sleep(0.75)
            print("\n%s:師父，這裡有沒有什麼事是我可以幫忙的" % (player.name))
            time.sleep(0.75)
            print("\n師父:%s啊，你願意幫忙讓為師感到非常欣慰阿" % (player.name))
            time.sleep(0.75)
            print("\n師父:為師實在是太忙了，這裡有許多事為師都來不及做，既然%s你願意幫忙，那為師就把這幾件事交給你做了" % (player.name))
            time.sleep(0.75)
            for i in range(1, 6):
                quest_list.append(
                    "\n%s/%s/%s"
        ...
        if option_2 == "5":
            time.sleep(0.5)
            print("\n%s:師父你死了沒阿(嘻皮笑臉)" % (player.name))
            time.sleep(1)
            print("\n師父:......(默默拿出菜刀)")
            time.sleep(1)
            print("\n%s:徒兒只是開個玩笑而已阿!!!(怕)" % (player.name))
            time.sleep(2)
            print("\n師父:既然你這麼閒，為師就來好好陪你鍛鍊一下(暴怒)")
            time.sleep(1.5)
            print("\n%s:師父不要阿!!!(怕)" % (player.name))
            time.sleep(1.5)
            # 名稱，境界，修為，攻擊，防禦，閃避，血量
            fa = Fighter("暴怒的師父", 99, 99999999, 999999, 999999, 999999, 999999)
            fb = Fighter(player.name, player.realm, player.cultivation, player.attack, player.defense, player.dodge, player.max_health)
            battle(fa, fb)
            time.sleep(1.5)
            print("\n師父:%s還是太頑皮了，還須打磨阿(嘆氣)" % (player.name))
            time.sleep(1.5)
            option_2 = 0
        if option_2 == "6":
            time.sleep(0.75)
    ...
    while option_1 == "3":
        ...
        while option_2 == "2":
            ...
            if option_3 == "1":
                ...
                while b > 0:
                    ...
                    print("%s支付了10元休息了一天，因為太過懶散，修為下降%d" % (player.name, c))
                    player.cultivation = player.cultivation - c
                    player.money -= 10
                    b -= 1
                print("\n%s離開了%s" % (player.name, place))
            if option_3 == "2":
                menu_list_0 = ["編號", "菜名", "價格", "介紹"]
                menu_list_1: list[int | str] = [1, "", "", "\t"]
                menu_list_2: list[int | str] = [2, "", "", "\t"]
                menu_list_3: list[int | str] = [3, "", "", "\t"]
                menu_list_4: list[int | str] = [4, "", "", "\t"]
                menu_list_5: list[int | str] = [5, "", "", "\t"]
        ...
        if option_2 == "4":
            ...
            print("\n%s進入了%s" % (player.name, place))
            product_list_0 = ["編號", "商品價格", "商品詳情"]
            product_list_1: list[int | str] = [1, "", "", "\t"]
            product_list_2: list[int | str] = [2, "", "", "\t"]
            product_list_3: list[int | str] = [3, "", "", "\t"]
            product_list_4: list[int | str] = [4, "", "", "\t"]
            product_list_5: list[int | str] = [5, "", "", "\t"]
            product_list = ["\n%s/%s/%s" % (product_list_0[0], product_list_0[1], product_list_0[2])]
            for i in range(1, 6):
                product_list.append(
                    "%s/%s/%s"
